...
void plot_position_map(int XePeriod, double binWidth)
{
  ...
  TString swankfilename = TString::Format("PMT_MAP_Swank_Array_XePeriod_%i_%0.1fmm.root",XePeriod,binWidth);
  ...
  for (int xb=0; xb<nPosBinsXY; xb++) {
	...
	std::vector <Double_t> eta = swankmap.getInterpolatedEta(plotMap.getBinCenter(xb),plotMap.getBinCenter(yb),plotMap.getBinCenter(xb),plotMap.getBinCenter(yb));
	//std::vector <Double_t> eta = swankmap.getInterpolatedEta(0., 0., 0., 0.);
  ...
  filenameOut = "PMT_MAP_Swank_Array_XePeriod_"+TString(runPeriodString)+TString::Format("_%0.1fmm.pdf",binWidth);
