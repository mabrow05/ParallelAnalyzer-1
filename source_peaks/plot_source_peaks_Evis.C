...
void plot_source_peaks_Evis(TString runNumber)
{
  ...
  // Open input ntuple
  TString filenameIn;
  filenameIn  = TString(getenv("SOURCE_PEAKS"))+TString("/source_peaks_EvisPMTbyPMT_");
  ...
  // Output file
  TString filenameOut;
  filenameOut  = TString(getenv("SOURCE_PEAKS"))+TString("/source_peaks_EvisPMTbyPMT_");
  ...
  if (nSources > 1 && useSource[1]) {
    double upperRange = 600.;
    if (sourceName[1]=="Bi") upperRange = 1200.;
  ...
  if (nSources > 2 && useSource[2]) {
    double upperRange = 600.;
    if (sourceName[2]=="Bi") upperRange = 1200.;
