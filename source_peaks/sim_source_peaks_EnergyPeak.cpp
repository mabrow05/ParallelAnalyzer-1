...
int main(int argc, char *argv[])
{
  ...
  // Open output ntuple
  char tempOut[500];
  sprintf(tempOut, "%s/source_peaks/source_peaks_EnergyPeak_%s.root",getenv("REVCALSIM"), argv[1]);
  ...
  // Loop over events
    for (int i=0; i<nEvents; i++) {
      ...
      if (side == 0) {
	his[src][0]->Fill(Evis[0]);
      }
      if (side == 1) {
	his[src][1]->Fill(Evis[1]);
      }	
    }
    fileIn->Close();
  }
  ...
  sprintf(tempResults, "%s/source_peaks/source_peaks_EnergyPeak_%s.dat",getenv("REVCALSIM"), argv[1]);
