...
std::vector <std::vector <double> > returnSourcePosition (Int_t runNumber, std::string src) {
  Char_t temp[500];
...
void plot_resid_vs_pos(Int_t calPeriodLow, Int_t calPeriodHigh,string SRC) {
  ...
  //East Residuals
  sprintf(tempfile, "%s/residuals/global_residuals_Erecon_runPeriods_%i-%i.dat",getenv("ANALYSIS_CODE"), calPeriodLow, calPeriodHigh); 
  ...
  for (UInt_t i=0; i<run.size(); i++) {
    std::vector < std::vector<Double_t> > pos = returnSourcePosition(run[i],source[i].substr(0,2));
  ...
  c1->Print(TString::Format("residualPosDep_%i-%i_%s.pdf(",calPeriodLow,calPeriodHigh,SRC.c_str()));
  c2->Print(TString::Format("residualPosDep_%i-%i_%s.pdf)",calPeriodLow,calPeriodHigh,SRC.c_str()));
}
