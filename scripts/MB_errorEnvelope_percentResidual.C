...
void MB_errorEnvelope_percentResidual(Int_t calLow, Int_t calHigh, Int_t pmt)
{
  ...
  // Histograms
  TH1F *hisCeEast = new TH1F("hisCeEast", "", 30, -0.060, 0.060);
  TH1F *hisSnEast = new TH1F("hisSnEast", "", 30, -0.060, 0.060);
  TH1F *hisBiEast = new TH1F("hisBiEast", "", 30, -0.060, 0.060);
  TH1F *hisCeWest = new TH1F("hisCeWest", "", 30, -0.060, 0.060);
  TH1F *hisSnWest = new TH1F("hisSnWest", "", 30, -0.060, 0.060);
  TH1F *hisBiWest = new TH1F("hisBiWest", "", 30, -0.060, 0.060);
  TH1F *hisCe = new TH1F("hisCe", "", 30, -0.060, 0.060);
  TH1F *hisSn = new TH1F("hisSn", "", 30, -0.060, 0.060);
  TH1F *hisBi = new TH1F("hisBi", "", 30, -0.060, 0.060);
  

  // Fill histograms
  for (int j=0; j<nCeEast; j++) {
    hisCeEast->Fill(resCeEast[j]/peakCe);
    hisCe->Fill(resCeEast[j]/peakCe);
  }
  for (int j=0; j<nSnEast; j++) {
    hisSnEast->Fill(resSnEast[j]/peakSn);
    hisSn->Fill(resSnEast[j]/peakSn);
  }
  for (int j=0; j<nBiEast; j++) {
    hisBiEast->Fill(resBiEast[j]/peakBiHigh);
    hisBi->Fill(resBiEast[j]);
  }
  for (int j=0; j<nCeWest; j++) {
    hisCeWest->Fill(resCeWest[j]/peakCe);
    hisCe->Fill(resCeWest[j]/peakCe);
  }
  for (int j=0; j<nSnWest; j++) {
    hisSnWest->Fill(resSnWest[j]/peakSn);
    hisSn->Fill(resSnWest[j]/peakSn);
  }
  for (int j=0; j<nBiWest; j++) {
    hisBiWest->Fill(resBiWest[j]/peakBiHigh);
    hisBi->Fill(resBiWest[j]/peakBiHigh);
  }
 
  // Calculate mean and standard deviation
  double meanCeEast = 0.;
  for (int j=0; j<nCeEast; j++) {
    meanCeEast += resCeEast[j]/peakCe;
  }
  meanCeEast = meanCeEast / (double) nCeEast;

  double sigmaCeEast = 0.;
  for (int j=0; j<nCeEast; j++) {
    sigmaCeEast += (resCeEast[j]/peakCe - meanCeEast)*(resCeEast[j]/peakCe - meanCeEast);
  }
  ...
  double meanSnEast = 0.;
  for (int j=0; j<nSnEast; j++) {
    meanSnEast += resSnEast[j]/peakSn;
  }
  meanSnEast = meanSnEast / (double) nSnEast;

  double sigmaSnEast = 0.;
  for (int j=0; j<nSnEast; j++) {
    sigmaSnEast += (resSnEast[j]/peakSn - meanSnEast)*(resSnEast[j]/peakSn - meanSnEast);
  }
  ...
  double meanBiEast = 0.;
  for (int j=0; j<nBiEast; j++) {
    meanBiEast += resBiEast[j]/peakBiHigh;
  }
  meanBiEast = meanBiEast / (double) nBiEast;

  double sigmaBiEast = 0.;
  for (int j=0; j<nBiEast; j++) {
    sigmaBiEast += (resBiEast[j]/peakBiHigh - meanBiEast)*(resBiEast[j]/peakBiHigh - meanBiEast);
  }
  ...
  double meanCeWest = 0.;
  for (int j=0; j<nCeWest; j++) {
    meanCeWest += resCeWest[j]/peakCe;
  }
  meanCeWest = meanCeWest / (double) nCeWest;

  double sigmaCeWest = 0.;
  for (int j=0; j<nCeWest; j++) {
    sigmaCeWest += (resCeWest[j]/peakCe - meanCeWest)*(resCeWest[j]/peakCe - meanCeWest);
  }
  ...
  double meanSnWest = 0.;
  for (int j=0; j<nSnWest; j++) {
    meanSnWest += resSnWest[j]/peakSn;
  }
  meanSnWest = meanSnWest / (double) nSnWest;

  double sigmaSnWest = 0.;
  for (int j=0; j<nSnWest; j++) {
    sigmaSnWest += (resSnWest[j]/peakSn - meanSnWest)*(resSnWest[j]/peakSn - meanSnWest);
  }
  ...
  double meanBiWest = 0.;
  for (int j=0; j<nBiWest; j++) {
    meanBiWest += resBiWest[j]/peakBiHigh;
  }
  meanBiWest = meanBiWest / (double) nBiWest;

  double sigmaBiWest = 0.;
  for (int j=0; j<nBiWest; j++) {
    sigmaBiWest += (resBiWest[j]/peakBiHigh - meanBiWest)*(resBiWest[j]/peakBiHigh - meanBiWest);
  }
  ...
  double meanCe = 0.;
  for (int j=0; j<nCeEast; j++) {
    meanCe += resCeEast[j]/peakCe;
  }
  for (int j=0; j<nCeWest; j++) {
    meanCe += resCeWest[j]/peakCe;
  }
  meanCe = meanCe / (double) (nCeEast+nCeWest);

  double sigmaCe = 0.;
  for (int j=0; j<nCeEast; j++) {
    sigmaCe += (resCeEast[j]/peakCe - meanCe)*(resCeEast[j]/peakCe - meanCe);
  }
  for (int j=0; j<nCeWest; j++) {
    sigmaCe += (resCeWest[j]/peakCe - meanCe)*(resCeWest[j]/peakCe - meanCe);
  }
  ...
  double meanSn = 0.;
  for (int j=0; j<nSnEast; j++) {
    meanSn += resSnEast[j]/peakSn;
  }
  for (int j=0; j<nSnWest; j++) {
    meanSn += resSnWest[j]/peakSn;
  }
  meanSn = meanSn / (double) (nSnEast+nSnWest);

  double sigmaSn = 0.;
  for (int j=0; j<nSnEast; j++) {
    sigmaSn += (resSnEast[j]/peakSn - meanSn)*(resSnEast[j]/peakSn - meanSn);
  }
  for (int j=0; j<nSnWest; j++) {
    sigmaSn += (resSnWest[j]/peakSn - meanSn)*(resSnWest[j]/peakSn - meanSn);
  }
  ...
  double meanBi = 0.;
  for (int j=0; j<nBiEast; j++) {
    meanBi += resBiEast[j]/peakBiHigh;
  }
  for (int j=0; j<nBiWest; j++) {
    meanBi += resBiWest[j]/peakBiHigh;
  }
  meanBi = meanBi / (double) (nBiEast+nBiWest);

  double sigmaBi = 0.;
  for (int j=0; j<nBiEast; j++) {
    sigmaBi += (resBiEast[j]/peakBiHigh - meanBi)*(resBiEast[j]/peakBiHigh - meanBi);
  }
  for (int j=0; j<nBiWest; j++) {
    sigmaBi += (resBiWest[j]/peakBiHigh - meanBi)*(resBiWest[j]/peakBiHigh - meanBi);
  }
  ...
  hisCeEast->SetXTitle("East Ce E_{Q} Error [%]");
  hisCeEast->GetXaxis()->CenterTitle();
  hisCeEast->SetLineColor(1);
  hisCeEast->Draw();
  hisCeEast->Fit("gaus", "", "", -0.040, 0.040);
  ...
  hisSnEast->SetXTitle("East Sn E_{Q} Error [%]");
  hisSnEast->GetXaxis()->CenterTitle();
  hisSnEast->SetLineColor(1);
  hisSnEast->Draw();
  hisSnEast->Fit("gaus", "", "", -0.040, 0.040);
  ...
  hisBiEast->SetXTitle("East Bi E_{Q} Error [%]");
  hisBiEast->GetXaxis()->CenterTitle();
  hisBiEast->SetLineColor(1);
  hisBiEast->Draw();
  hisBiEast->Fit("gaus", "","", -0.04, 0.04);
  ...
  hisCeWest->SetXTitle("West Ce E_{Q} Error [%]");
  hisCeWest->GetXaxis()->CenterTitle();
  hisCeWest->SetLineColor(1);
  hisCeWest->Draw();
  hisCeWest->Fit("gaus", "", "", -0.04, 0.04);
  ...
  hisSnWest->SetXTitle("West Sn E_{Q} Error [%]");
  hisSnWest->GetXaxis()->CenterTitle();
  hisSnWest->SetLineColor(1);
  hisSnWest->Draw();
  hisSnWest->Fit("gaus", "", "", -0.04, 0.04);
  cout << "meanCeWest = " << gaus->GetParameter(1) << endl;
  cout << "     sigma = " << gaus->GetParameter(2)  << endl;
  errEnv << "meanCeWest = " << gaus->GetParameter(1) << endl;
  errEnv << "sigma = " << gaus->GetParameter(2) << endl;
  ...
  hisBiWest->SetXTitle("West Bi E_{Q} Error [%]");
  hisBiWest->GetXaxis()->CenterTitle();
  hisBiWest->SetLineColor(1);
  hisBiWest->Draw();
  hisBiWest->Fit("gaus", "", "", -0.04, 0.04);
  ...
  hisCe->SetXTitle("Total Ce E_{Q} Error [%]");
  hisCe->GetXaxis()->CenterTitle();
  hisCe->SetLineColor(1);
  hisCe->Draw();
  hisCe->Fit("gaus", "", "", -0.040, 0.040);
  ...
  hisSn->SetXTitle("Total Sn E_{Q} Error [%]");
  hisSn->GetXaxis()->CenterTitle();
  hisSn->SetLineColor(1);
  hisSn->Draw();
  hisSn->Fit("gaus", "", "", -0.040, 0.040);
  ...
  hisBi->SetXTitle("Total Bi E_{Q} Error [%]");
  hisBi->GetXaxis()->CenterTitle();
  hisBi->SetLineColor(1);
  hisBi->Draw();
  hisBi->Fit("gaus", "", "", -0.04, 0.04);
