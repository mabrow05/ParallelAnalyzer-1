...
int main(int argc, char *argv[])
{
  ...
  if ((int)referenceRun!=runNumber) {
    sprintf(tempIn,"%s/gain_LED_%i.dat",getenv("GAIN_LED"),referenceRun);
  ...
  // Open output ntuple
  char tempOut[500];
  sprintf(tempOut, "%s/gain_LED_%s.root",getenv("GAIN_LED"),argv[1]);
  ...
  std::vector <Int_t> nLEDEvents(8,0);
  ...
  for (int i=0; i<nEvents; i++) {
    ...
    if (ScintE.q1>500.) nLEDEvents[0]++;
    if (ScintE.q2>500.) nLEDEvents[1]++;
    if (ScintE.q3>500.) nLEDEvents[2]++;
    if (ScintE.q4>500.) nLEDEvents[3]++;
    if (ScintW.q1>500.) nLEDEvents[4]++;
    if (ScintW.q2>500.) nLEDEvents[5]++;
    if (ScintW.q3>500.) nLEDEvents[6]++;
    if (ScintW.q4>500.) nLEDEvents[7]++;
  }
  ...
  for (int j=0; j<8; j++) {
    ...
    int maxCounter = 0;
    int minCounter = 0;
    cout << nLEDEvents[j] << endl;
    Int_t fuzzCounts = 0.004*nLEDEvents[j];
  ...
  char tempFit[500];
  sprintf(tempFit, "%s/gain_LED_%s.dat", getenv("GAIN_LED"),argv[1]);
