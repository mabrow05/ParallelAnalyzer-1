...
void PlotFinalAsymmetries(std::string groupType, Int_t octBegin, Int_t octEnd, std::string anaChoice, Double_t Elow, Double_t Ehigh, 
  ...
  for (Int_t octet=octBegin; octet<=octEnd; octet++) {
	...
	if (infile.is_open()) {	  
	  ...
	  std::vector <Double_t> backscCorr = LoadBackscCorrections(enBinMedian,octet);
	  
	  Int_t i = 0;
	  while (infile >> binEdge >> Asym >> AsymError) {
	    groupRawAsymByBin[0][i] = binEdge;
	    groupRawAsymByBin[1][i] += ( AsymError>0. ? 1./power(AsymError*backscCorr[i]*angleCorr[i]*fieldCorr[i]*deltaSys[i][0]/theoryCorr[i],2)
					 *Asym*backscCorr[i]*angleCorr[i]*fieldCorr[i]*deltaSys[i][0]/theoryCorr[i] : 0. ); //Applying Delta_exp here.. Should this affect AsymError???
	    groupRawAsymByBin[2][i] += AsymError>0. ? 1/power(AsymError*backscCorr[i]*angleCorr[i]*fieldCorr[i]*deltaSys[i][0]/theoryCorr[i],2) : 0.;         // Since Asym is multiplied by delta, AsymError would be as well...
...
std::vector <Double_t> LoadAngleCorrections(std::vector <Double_t> enBinMidpoint,Int_t oct) {
  ...
  TString filename = TString::Format("angleCorr_%s.txt",oct<60?"2011-2012":"2012-2013");
  //std::cout << filename.Data() << std::endl;
...
std::vector <Double_t> LoadBackscCorrections(std::vector <Double_t> enBinMidpoint,Int_t oct) {
  std::vector <Double_t> syst(enBinMidpoint.size(), 1.);
  if ( corr.substr(0,11)!=std::string("DeltaBacksc") && corr!=std::string("AllCorr") ) return syst;
  std::string c = corr.substr(10,1);
  TString filename = TString::Format("%s_deltaBS_%s.txt",oct<60?"2011-2012":"2012-2013",
				     c==std::string("0")?"0":
				     c==std::string("1")?"1":
				     c==std::string("2")?"2":
